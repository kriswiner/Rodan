...
void CCS811::checkCCS811Status() 
{
  ...
  checkCCS811Status();
        ...
        Wire1.transfer(CCS811_ADDRESS, &temp[0], 1, NULL, 0); 
        delay(100);
  checkCCS811Status();
  ...
  void CCS811::compensateCCS811(int16_t compHumidity, int16_t compTemp)
  {
      // Update CCS811 humidity and temperature compensation
      uint8_t temp[5] = {0, 0, 0, 0, 0};
      temp[0] = CCS811_ENV_DATA;
      temp[1] = ((compHumidity % 100) / 100) > 7 ? (compHumidity/100 + 1)<<1 : (compHumidity/100)<<1;
      temp[2] = 0;
      if(((compHumidity % 100) / 100) > 2 && (((compHumidity % 100) / 100) < 8))
      {
       temp[1] |= 1;
      }
      ...
      Wire1.transfer(CCS811_ADDRESS, &temp[0], 5, NULL, 0);
